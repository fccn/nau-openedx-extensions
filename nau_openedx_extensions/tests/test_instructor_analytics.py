...
class InstallTest(TestCase):
    ...
    def test_missing_task_is_logged(self):
        """An upstream rename of upload_students_csv must be visible in the logs, not skipped silently."""
        from lms.djangoapps.instructor_task import tasks  # pylint: disable=import-error,import-outside-toplevel

        with patch.dict(tasks.__dict__), self.assertLogs(
            "nau_openedx_extensions.utils.instructor_analytics", level="WARNING"
        ) as logs:
            del tasks.__dict__["upload_students_csv"]
            install_upload_students_csv_wrapper()

        self.assertIn(tasks.__name__, logs.output[0])
