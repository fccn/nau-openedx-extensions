...
Fields are gated by the Django allowlist and the course advanced setting (see
``get_nau_additional_course_features``), and by a third, platform-level gate:
a field listed in ``PROFILE_INFORMATION_REPORT_PRIVATE_FIELDS`` is never
exported, whatever the other two say.
"""
...
def install_upload_students_csv_wrapper():
    ...
    for module in (enrollments, tasks):
        ...
        if current is None:
            logger.warning(
                "upload_students_csv not found in %s; NAU profile columns wrapper not installed there.",
                module.__name__,
            )
            continue
        if getattr(current, "_nau_additional_course_features", False):
            continue
