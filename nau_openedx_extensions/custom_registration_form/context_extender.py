...
def partial_update(update, user, **kwargs):
    ...
    if "extended_profile" in update:
        ...
        editable_fields = {field.name for field in get_fields(custom_model_instance)}
        ...
        for field in new_extended_profile:
            ...
            if field_name not in editable_fields:
                # Only the fields the account page is allowed to show can be written
                # through it. Without this check any account PATCH could set any
                # field on the model, including the cc_* data that comes from
                # Autenticacao Gov and the data_authorization consent flag.
                log.warning(
                    "Ignoring extended profile field %s for user %s, it is not in "
