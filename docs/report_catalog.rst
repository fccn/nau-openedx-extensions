...
**Pre-flight check before enabling.** Do not set
``NAU_REPORTS_ENABLE_BASE_COLUMNS`` or ``NAU_REPORTS_ENABLE_CERTIFICATE_DATE``
to ``True`` in any real environment until ARTE has confirmed, in
fccn/nau-technical#955, that its pipeline reads these CSVs by header name.
Both flags change the column layout, and a position-based reader would break
silently rather than fail. At the time of writing this is still unanswered.
